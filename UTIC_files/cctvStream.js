...
function StreamCctv(cctvid, mapType){
	...
	var debugFlag = true;
	//gCctv = window.open.postMessage("cross domain success",viewCctv);
	gCctv = window.opener.viewCctv;
...
//	}
	...
	this.getImageHtml = function(){
		...
		if(this.gMovie == 'U'){//유고처리
			...
			var features = 'width='+ width +',height='+ height +',top='+ top +',left='+ left +',toolbar=no,scrollbars=no,status=no,location=no';
	...
	//return "<div id='Layer1' style='z-index: 1; position: absolute; top: 0px; left: 0px; overflow: hidden; width: 320; height: 240; float: left; margin-left: 10px;'>"
	//    + "<iframe src='http://urrs.wonju.go.kr/its/instorm/viewer.html?deviceid=" + this.gPort + "' frameborder='0' width='320' height='240' scrolling='no'  />"
	//   + "</div>";
	//};
	...
	this.cctvHTML_Seoul = function(){
		...
		/*var id = this.gCctvId.substring(4,7);
		var cid = this.gId;
		...
		function setLogs(p) {
			 ...
			 document.getElementById('logifrm').src = 'http://61.108.209.107:88/log_kroad.html?pnum='+p;
		}

		function log_kroad(pn) {

			$.ajax({

				url: 'http://192.168.202.107:88/log_kroad.html',
				data: 'pnum=' + pn,
				success: function(){
				},
				error: function(){
		...
		var SrcStreaming = '<iframe src="http://www.utic.go.kr/contents/js/Component/_KoRoad.html?'
	         + 'cctvid=' + this.gId
	         + '&ip=' + gCctv.CCTVIP
	         + '" id="SeoulCCTV"class="CCTVFrame"name="SeoulView"frameborder="0"scrolling="no"title="서울CCTV"'
	         + 'width="320"height="245"marginheight="0"marginwidth="0"align="middle"style="margin: 0;padding: 0;"/>';
		return SrcStreaming;*/
	};
